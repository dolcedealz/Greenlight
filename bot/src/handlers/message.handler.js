...
function registerMessageHandlers(bot) {
  ...
  bot.hears('👥 Рефералы', async (ctx) => {
    try {
      ...
      await ctx.reply(
        ...
        `💰 От 5% до 15% с проигрышей ваших рефералов\n\n` +
        `📊 Уровни партнерской программы:\n` +
        `🥉 Бронза (0+ рефералов) - 5%\n` +
        `🥈 Серебро (6+ активных) - 7%\n` +
        `🥇 Золото (21+ активных) - 10%\n` +
        `💎 Платина (51+ активных) - 12%\n` +
        `🌟 VIP (101+ активных) - 15%\n\n` +
        `🔗 Ваша реферальная ссылка:\n` +
        `${referralLink}\n\n` +
        `💡 Комиссия начисляется только с проигрышей рефералов\n` +
        `🎁 Бонус 1 USDT за первый депозит каждого реферала\n\n` +
  ...
  bot.hears('🎁 Розыгрыши', async (ctx) => {
    const webAppData = getWebAppUrl('?tab=giveaways');
    
    if (webAppData.isValid) {
      await ctx.reply(
        '🎁 *Розыгрыши призов*\n\n' +
        '🏆 Ежедневные и недельные розыгрыши\n' +
        '💎 Ценные призы и Telegram Gifts\n' +
        '🎯 Честные результаты через Telegram кубик\n\n' +
        '📋 *Как участвовать:*\n' +
        '• Сделайте депозит от 1 USDT\n' +
        '• Откройте раздел "Розыгрыши" в приложении\n' +
        '• Нажмите кнопку "Участвовать"\n\n' +
        '⏰ Розыгрыши проводятся ежедневно в 20:00 МСК',
        {
          parse_mode: 'Markdown',
          reply_markup: Markup.inlineKeyboard([
            [
              Markup.button.webApp('🎁 Открыть розыгрыши', webAppData.url)
            ],
            [
              Markup.button.callback('ℹ️ Правила участия', 'giveaway_rules')
            ]
          ]).reply_markup
        }
      );
      ...
      await ctx.reply(
        ...
        'Используйте команду /giveaways для получения информации о текущих розыгрышах',
        { parse_mode: 'Markdown' }
      );
    }
  });
