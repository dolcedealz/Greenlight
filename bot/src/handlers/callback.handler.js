...
function registerCallbackHandlers(bot) {
  ...
  bot.action('giveaway_rules', async (ctx) => {
    try {
      await ctx.answerCbQuery();
      ...
      await ctx.reply(
        '📋 *Правила участия в розыгрышах*\n\n' +
        '🎯 *Типы розыгрышей:*\n' +
        '• 🏆 Ежедневные - каждый день в 20:00 МСК\n' +
        '• 💎 Недельные - каждое воскресенье в 20:00 МСК\n' +
        '• 🎯 Кастомные - по расписанию администратора\n\n' +
        '✅ *Условия участия:*\n' +
        '• Сделайте депозит требуемой суммы\n' +
        '• Для ежедневных: депозит в тот же день\n' +
        '• Для недельных: депозит в течение недели\n' +
        '• Нажмите "Участвовать" в приложении\n\n' +
        '🎲 *Как определяются победители:*\n' +
        '• Используется Telegram кубик для честности\n' +
        '• Результат виден всем в канале\n' +
        '• Невозможно подтасовать результаты\n\n' +
        '🏆 *Получение призов:*\n' +
        '• Победители получают уведомления\n' +
        '• Результаты публикуются в канале\n' +
        '• Связывайтесь с поддержкой для получения приза',
        { parse_mode: 'Markdown' }
      );
    } catch (error) {
      console.error('Ошибка показа правил розыгрышей:', error);
