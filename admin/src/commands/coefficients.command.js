...
async function setupGlobalGameCoefficient(ctx, gameType) {
  ...
  // ИСПРАВЛЕНО: Разные описания для разных игр
  const getModifierDescription = (gameType) => {
    switch (gameType) {
      case 'coin':
        return `• Положительное число - увеличивает шанс выигрыша игрока (легче)\n` +
               `• Отрицательное число - уменьшает шанс выигрыша игрока (сложнее)\n` +
               `• 0 - стандартный шанс 47.5%`;
      case 'slots':
        return `• Положительное число - увеличивает RTP (больше выигрышей)\n` +
               `• Отрицательное число - уменьшает RTP (меньше выигрышей)\n` +
               `• 0 - стандартный RTP 90%`;
      case 'mines':
        return `• Положительное число - больше мин на поле (сложнее)\n` +
               `• Отрицательное число - меньше мин на поле (легче)\n` +
               `• 0 - стандартное количество мин`;
      case 'crash':
        return `• Положительное число - больше ранних крашей (сложнее)\n` +
               `• Отрицательное число - меньше ранних крашей (легче)\n` +
               `• 0 - стандартное распределение`;
      default:
        return `• Положительное число - увеличивает сложность для игрока\n` +
  ...
  const message = `🎯 *Настройка ${gameNames[gameType]}*\n\n` +
    ...
    getModifierDescription(gameType) + `\n\n` +
